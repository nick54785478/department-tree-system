 ...
 *
 * <pre>
 * 專為高效能讀取設計，負責執行原生 SQL 並將 ResultSet 映射至 DTO。 ⚠️ 架構安全約定：所有的查詢皆已全面加上邏輯刪除過濾條件
 * ({@code status != 'DELETED'})。 這是為了確保在時光機架構下，被標記為刪除但尚未被物理清除的幽靈節點，不會外洩給前端顯示。
 * </pre>
 */
@Repository
...
class DepartmentTreeReaderAdapter implements DepartmentTreeReaderPort {
	 ...
	 */
	@Override
	public List<DepartmentNode> getSubtree(String tenantId, String rootDepartmentId) {
		String sql = """
				SELECT
				    v.tenant_id,
				...
				ORDER BY dt.depth ASC, v.sort_order ASC
				""";
